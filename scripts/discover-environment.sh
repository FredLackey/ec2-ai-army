...
usage() {
    cat << EOF
Usage: $0 -p|--profile <aws-profile> -r|--region <aws-region> [-o|--output <output-file>] [-w|--overwrite]

Discover AWS environment information for Moodle deployment.

Required arguments:
  -p, --profile <profile>    AWS SSO profile name (e.g., corp-dev, corp-prod, corp-sandbox, corp-mgmt)
  -r, --region <region>      AWS region (e.g., us-east-1, us-west-2)

Optional arguments:
  -o, --output <file>        Output JSON file path (default: aws-environment.json)
  -w, --overwrite           Overwrite existing output file (default: reuse existing file if present)
  -h, --help                Show this help message

Examples:
  $0 -p corp-dev -r us-east-1
  $0 --profile corp-dev --region us-east-1
  $0 -p corp-prod -r us-east-1 -o prod-env.json -w
  $0 --profile corp-prod --region us-east-1 --output prod-env.json --overwrite
  $0 -p corp-dev -r us-east-1  # Reuses existing aws-environment.json if present
EOF
}
